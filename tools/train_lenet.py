...
@click.option('--snapshot', default=500)
...
def main(dataset, split, model, output, gpu, iterations, batch_size, display,
    ...
    selected = list(range(10))#[0,2]
    dataset_name = dataset
    split_name = split
    dataset = getattr(adda.data.get_dataset(dataset, selected=selected), split)
    ...
    net, layers = lenet(im_batch, n_classes=len(selected))
