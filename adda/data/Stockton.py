...
@register_dataset('Stockton')
class Stockton(DatasetGroup):
    '''Stockton_training: chunks 1-9 (100008 panel:non_panel=22661:77347)
       Fresno_testing: chunk 12 (20000 panel:non_panel=2466:17534)'''


    data_files={'train':'Stockton_training.mat','test':'Stockton_testing.mat'}
    num_classes=2

    def __init__(self, path='/home/ray/adda-master/data/solar_panel',
                 shuffle=True):
        DatasetGroup.__init__(self, 'Stockton', path=path)
    ...
    def _load_datasets(self):
        ...
        self.test = ImageDataset(test_images, test_labels,
                                  ...
                                  label_shape=self.label_shape,
                                  shuffle=self.shuffle)
        # # non_panel:panel=500:1
        # indices_panel=[i for i in range(len(train_labels)) if train_labels[i]==1]
        # indices_non_panel = [i for i in range(len(train_labels)) if train_labels[i] == 0]
        # ratio=10
        # interval=round( len(indices_panel)/( len(indices_non_panel)/ratio ) )
        # indices_panel_s = [indices_panel[i] for i in range( 0,len(indices_panel),interval )] #indices_panel[ range( 0,len(indices_panel),interval ) ]
        # indices_s = indices_non_panel + indices_panel_s
        # self.train = ImageDataset(train_images[indices_s], train_labels[indices_s],
        #                                   image_shape=self.image_shape,
        #                                   label_shape=self.label_shape,
        #                                   shuffle=self.shuffle)
